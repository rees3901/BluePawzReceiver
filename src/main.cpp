...
void logMessage(const JsonDocument &doc, const String &type)
{
  ...
  // V3.7.3: source / destination / message-ID attribution, set AFTER the copy
  // so they are authoritative. "lora"/"lora-ack" are INBOUND from a collar to
  // the base; every other type (event / mydevice / outbound command) ORIGINATES
  // at the base station. This is what lets a phantom log entry be traced to the
  // transmitter (src = a collar UID), the receiver (src = BASE_ID), or an
  // internal/base-generated message.
  uint16_t aSrc = BASE_ID, aDst = BASE_ID;
  if (type == "lora" || type == "lora-ack")
  {
    if (doc["device_id"].is<int>())
      aSrc = (uint16_t)doc["device_id"].as<int>(); // from this collar
    aDst = BASE_ID;                                 // to the base station
  }
  else
  {
    aSrc = BASE_ID; // generated by the base (event / mydevice / outbound cmd)
    if (doc["target_id"].is<int>())
  ...
  logEntry["src"] = aSrc;
  logEntry["dst"] = aDst;
  // Keep the packet/command's own msg_id when present; otherwise stamp a
  // monotonic sequential id so every entry is individually addressable.
...
void updateNodeState(const JsonDocument &doc)
{
  ...
  if (doc["ack"].is<String>())
  {
    ...
    // V3.7.3: a set_name ACK proves the collar APPLIED the name (to RAM) and
    // acknowledged — but NOT that it PERSISTED it across deep sleep. Marking it
    // DELIVERED here is the false positive being removed: the card/command would
    // flip to "delivered" ~2 s after submit, then the collar's next report
    // shows the OLD name (it never stuck). A rename is only truly DELIVERED once
    // the collar's own TELEMETRY carries the new name — confirmRenameByTelemetry
    // does exactly that. So leave the command pending on the ACK; telemetry
    // confirms it (DELIVERED) or the age backstop fails it if it never sticks.
    if (ackType == "set_name")
    {
      Serial.printf("[ACK] set_name applied on collar (msg_id=%lu) — NOT marked delivered until telemetry reports the new name\n", msgId);
      broadcastNodeStates();
      return;
    }
